    ...
    public static class MonobankAPI
    {
        ...
        public static async Task<List<Currency>> GetMonoCurrencies()
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                List<Currencies> myCurrencyList = await App.NotesDB.SelectAllAsyncFrom<Currencies,int>();
        ...
        public static async Task<int> GetStatementAndSave(DateTime date, int account = 0)
        {
            ...
            {
                var operation = await App.NotesDB.SelectAsyncFrom<CashFlowOperations,string>(statement.id,name: "MonoId");
                ...
                else
                {
                    ...
                    if (operation == null)
                    {
                        ...
                        {
                            var MCCCode = await App.NotesDB.SelectAsyncFrom<MCCCodes,string>(newOperation.MCC, name: "MCC");
                        ...
                        {
                            var ClientDescr = await App.NotesDB.SelectAsyncFrom<ClientIdentificationTexts,string>(newOperation.TextToIdentifyClient, name: "Description");
                        ...
                        {
                            var client = await App.NotesDB.SelectAsyncFrom<Clients,Guid>(newOperation.Client);
                            if (client != null)
